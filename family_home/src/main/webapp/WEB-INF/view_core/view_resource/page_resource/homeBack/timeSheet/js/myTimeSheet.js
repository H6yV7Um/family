...
$(document).ready(function() {
    ...
    $('#calendar').fullCalendar({
        ...
        dayClick: function(start, end) {
            ...
            sheetInfo = layer.open({
                ...
                yes: function(index, layero){
                    if(timeSheetCheck.call(this))
                        saveTimeSheet.call(this,index,layero);
                }
            });
        },
        //选中事件->修改
        eventClick: function(calEvent, jsEvent, view) {
            ...
            if(calEvent.assessFlag == "2")
                openReadTimeSheet.call(this,calEvent,jsEvent);
            else
                openEditTimeSheet.call(this,calEvent,jsEvent);
        },
        editable: true,
        ...
        events: function(start,end,timezone, callback) {
            family_ns.queryAjax({
                type:"post",
                url:family_ns.contextPath+"/timesheet/queryTimesheetList",
                data: {
 ...
 * 打开编辑的timeSheet
 */
function openEditTimeSheet(calEvent,jsEvent){
    initRtopic.call(this,calEvent);
    initBlogTableGrid.call(this,1,calEvent);
    sheetInfo = layer.open({
        type: 1,
        title: '工时信息',
        fix: false,
        ...
        success: function(layero, index){
            ...
            initTimeSheet.call(this,calEvent);
        },
        end: function(){
            //layer.tips('Hi', '#about', {tips: 1})
            cleanTimeSheet.call(this);
        ...
        yes: function(index, layero){
            if(timeSheetCheck.call(this))
                saveTimeSheet.call(this,calEvent,index,layer);
        },btn2: function(index, layero){
            deleteTimeSheet.call(this,index,layero);
            return false;
        }
    });
}
 ...
 * 保存
 */
function saveTimeSheet(calEvent){
    var formData = family_ns.serializeObject($("#sheetForm"));

    var rtopic = $('#rtopics').val();
    if(-1 != rtopic && formData.type == '2')
        formData.topicId = rtopic;
    var sheet_blog_Id = "";
    //获取checked menu
    $("#blogTableTbody input[name='blogCheck']:checked").each(function(){
        sheet_blog_Id += $(this).parent().attr("id") + ",";
    });
    formData.sheetBlogs = sheet_blog_Id;

    var url = family_ns.contextPath + "/timesheet/timesheetAdd";
    if($("#workBusinessId").val())
        url = family_ns.contextPath + "/timesheet/timesheetUpdate";
        ...
        family_ns.submitAjax({
            ...
            success:function(result){
                if(result.success){
                    ...
                    layer.close(sheetInfo);
                    //新增event
                    source[0] = result.result;
                    //修改
                    if($("#workBusinessId").val()){
                        calEvent = copyPropFromTimeSheet.call(this,result.result,calEvent);
...
function deleteTimeSheet(){
    var businessId = $("#workBusinessId").val();
    var url = family_ns.contextPath + "/timesheet/timesheetDelete/" + businessId;
    family_ns.operateConfirmByAjax(function(){
        family_ns.submitAjax({
            type:"post",
            url:url,
 ...
 * @param calEvent
 */
function resizeAndDropTimeSheet(calEvent,revertFunc){
    var formData = {};
    formData = copyPropFromEvent.call(this,formData,calEvent);
    var url = family_ns.contextPath + "/timesheet/timesheetUpdate";
    family_ns.operateConfirmByAjax(function(){
        family_ns.submitAjax({
            type:"post",
            url:url,
